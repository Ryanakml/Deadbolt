...
test("RFC 8785 JSON Canonicalization Scheme matches all golden vectors", () => {
  for (const vector of vectors) {
    ...
    assert.equal(
      ...
      `Canonical string mismatch for vector [${vector.id}]`,
    );
    assert.equal(
      sha256,
      vector.expectedSha256,
      `SHA-256 digest mismatch for vector [${vector.id}]`,
    );
  }
});

test("Rejection of invalid non-finite numbers per RFC 8785", () => {
  assert.throws(() => canonicalize({ bad: NaN }), { code: "INVALID_JSON" });
  assert.throws(() => canonicalize({ bad: Infinity }), {
    code: "INVALID_JSON",
  });
  assert.throws(() => canonicalize({ bad: -Infinity }), {
    code: "INVALID_JSON",
...
test("Rejection of undefined values per RFC 8785", () => {
  assert.throws(() => canonicalize({ bad: undefined }), {
    code: "INVALID_JSON",
  });
});
