...
func TestCleanDatabaseMigrationAsDeadboltMigrator(t *testing.T) {
	...
	if v != migrator.LatestSchemaVersion {
		t.Fatalf("expected latest migration version %d, got %d", migrator.LatestSchemaVersion, v)
	}
}
...
func TestMigrationAdvisoryLockBlocking(t *testing.T) {
	...
	// Verify version is now upgraded to latest
	v, err = runnerB.Version(ctx)
	if err != nil || v != migrator.LatestSchemaVersion {
		t.Fatalf("expected version %d after migration Up, got %d (err: %v)", migrator.LatestSchemaVersion, v, err)
	}
}
...
func TestIntermediateMigrationStepUpgradeSafety(t *testing.T) {
	...
	if err != nil || latestV != migrator.LatestSchemaVersion {
		t.Fatalf("expected version %d after Up, got %d (err: %v)", migrator.LatestSchemaVersion, latestV, err)
	}
