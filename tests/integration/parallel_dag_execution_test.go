...
func TestReconcile_SkippedPropagation(t *testing.T) {
	...
	_, states := queryRunStepStates(t, tc, orgID, runID)
...
// concurrently, the join waits for all success, and committed outputs map.
func TestParallelDiamond_RealAgent(t *testing.T) {
	...
	workflows := []map[string]any{{
		...
		"nodes": []map[string]any{
			...
			{"id": "d", "type": "task", "task": "agent-task", "after": []any{"b", "c"}},
		},
	...
	for time.Now().Before(deadline) {
		...
		if err == nil && snapshot.Status == contracts.RunStatusSUCCEEDED {
			...
			if output["value"] != "diamond-done" {
				t.Fatalf("unexpected diamond output: %+v", snapshot.Output)
			}
