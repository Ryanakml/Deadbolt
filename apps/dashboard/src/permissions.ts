...
export function roleCanControlRuns(role: string | null | undefined): boolean {
...
export function sessionCanControlRuns(
...
export function visibleRunControls(
  ...
  if (!canControl) return { pause: false, resume: false, cancel: false };
  switch (status) {
    case "QUEUED":
    case "RUNNING":
    case "WAITING":
