		...
		case <-ticker.C:
			...
			for _, stop := range hbRes.Stops {
				if stop.AttemptID == attemptID {
					...
					att := a.runningAttempts[attemptID]
					a.mu.RUnlock()
					// A runner may finish between the heartbeat request and the
					// server's stop response. Receipt must still be acknowledged:
					// no tracked process means the worker has already confirmed
					// that there is nothing left to terminate.
					stopped := true
					if att != nil && att.pid > 0 {
