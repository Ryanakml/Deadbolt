...
func (s *Service) CreateRun(
	...
	err = s.pool.WithTenantTx(ctx, orgID, func(ctx context.Context, tx storage.Tx) error {
		...
		// Check system recovery admission controls (Blueprint §27.3)
		var admissionEnabled bool = true
		var recMode string = "ACTIVE"
		_ = tx.QueryRow(ctx, `SELECT admission_enabled, mode FROM system_recovery_controls WHERE id = 1`).Scan(&admissionEnabled, &recMode)
