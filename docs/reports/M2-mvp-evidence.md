...
- Tests (new): `tests/integration/m2_gate_test.go`
  - `TestM2_TwoWorkerABCRecoveryKillDuringB` — central Blueprint §29.2
    A→B→C kill-worker recovery through the real Deadbolt path.
...
- Harness (new): `scripts/m2-gate.sh` — one repeatable entrypoint. Runs the
  new M2 tests plus orchestrates (not copies) the existing Issue #17–#24
  suites grouped by M2 case. Emits PASS/FAIL per group, exits non-zero on
  failure.
...
uses two worker processes (two authenticated worker sessions) on one host,
which proves worker death / ownership recovery semantics only. A
host-resilience claim requires workers in two separate failure domains
(Issue #25 contract) and is explicitly out of scope for this PR.
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| 1   | Two-worker A→B→C recovery: A succeeds, B starts on worker 1, worker 1 killed during B, ownership expires/fences, B follows safe-retry policy, worker 2 takes over with epoch+1 and stable operation ID, A not rerun, C runs after, final SUCCEEDED, API/DB/events/Inspector agree | #25 new (Blueprint §29.2; REQ-DUR-01; F-05) | `TestM2_TwoWorkerABCRecoveryKillDuringB` via `./scripts/m2-gate.sh` group `m2-gate-new`                                                                                                                                                                                                                                                                                                                                                                           | AUTOMATED_LOCAL_VERIFIED |
| 2   | Stale-worker rejection: after reassignment, old worker/session Start/heartbeat/Complete with stale ownership rejected                                                                                                                                                             | #17 + #25 central test                      | `TestWorkerReconnectRecoveryEndToEndSchedulable`, `TestWorkerClaimExpiryAndDurableRecovery`, `TestWorkerRunningLeaseExpiryAndDurableRecovery`, `TestWorkerSessionReauthenticationAndFencing`, `TestWorkerDBTimeBoundariesAndHeartbeatRequiresStart` + stale checks inside `TestM2_TwoWorkerABCRecoveryKillDuringB`; group `stale-worker`                                                                                                                          | AUTOMATED_LOCAL_VERIFIED |
...
`go test -v` log lines prefixed `M2-GATE`, `M2-UNKNOWN`, `M2-HTTP-LOCAL`
(e.g. central-test final line records `A=1/1 B=2/1 C=1/1` plus the full event
sequence `RUN_CREATED … RUN_COMPLETED`). IDs are per-run UUIDs; see CI log
for the exact values on the recorded HEAD. No run IDs are invented in this
report.
...
| F-05 | Worker dies mid-task                         | Lease expiry → retry/reconcile per policy                                             | `TestM2_TwoWorkerABCRecoveryKillDuringB` (central), `TestWorkerClaimExpiryAndDurableRecovery`, `TestWorkerRunningLeaseExpiryAndDurableRecovery`, `TestLeaseExpirySchedulesBackoffTimer`               | AUTOMATED                              | pending                                                 | PENDING                 |
| F-06 | Old worker alive after reassignment          | Stale Start/heartbeat/result rejected                                                 | Stale checks in central test + `TestWorkerReconnectRecoveryEndToEndSchedulable`                                                                                                                       | AUTOMATED                              | pending                                                 | PENDING                 |
