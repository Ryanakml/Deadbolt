...
- `choice`, `merge`, `approval`, and `delay` are reserved node types/fields. Their execution remains unavailable and `validateWorkflowManifest` rejects them with `UNSUPPORTED_CAPABILITY`. Static task-only DAGs, including parallel roots, fan-out, and all-success joins, are executable; the structural schema's V1 ceiling is 200 nodes while executable admission remains 50. This is not V1 structured-branch validation.
